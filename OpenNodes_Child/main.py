...
LANGUAGES = [
    {"code": "en", "name": "English-3-4", "file": "data/eng_3-4.txt", "splitter": r"(?<=[.!?;:])\s+|\n\r", "color": "#F7DC6F"},
    {"code": "en", "name": "English-4-5", "file": "data/eng_4-5.txt", "splitter": r"(?<=[.!?;:])\s+|\n\r", "color": "#9370DB"},
    {"code": "en", "name": "English-5-6", "file": "data/eng_5-6.txt", "splitter": r"(?<=[.!?;:])\s+|\n\r", "color": "#2E8B57"},
    {"code": "en", "name": "English-6-7", "file": "data/eng_6-7.txt", "splitter": r"(?<=[.!?;:])\s+|\n\r", "color": "#FF8C00"},
    {"code": "en", "name": "English-7-8", "file": "data/eng_7-8.txt", "splitter": r"(?<=[.!?;:])\s+|\n\r", "color": "#8B4513"},
    {"code": "en", "name": "English-8-9", "file": "data/eng_8-9.txt", "splitter": r"(?<=[.!?;:])\s+|\n\r", "color": "#1D8BFF"},
    {"code": "en", "name": "English-9-10", "file": "data/eng_9-10.txt", "splitter": r"(?<=[.!?;:])\s+|\n\r", "color": "#FFA757"}
]
...
def plot_single_language(name, code, color, models_data):
    # Set Nature style proportions, compact canvas
    fig, ax = plt.subplots(figsize=(4.5, 4), dpi=300)
    
    # Plot Human Working Memory Capacity (WMC) interval using neutral light gray
    ...
    for m_name, m_cfg in configs.items():
        ...
        # Scatter: small size and transparency to handle data overlap
...
def plot_combined_24_lines_with_points(all_lang_data):
    # Nature-style large figure, double-column width
    fig, ax = plt.subplots(figsize=(10, 7), dpi=300)
    
    # Plot Human Working Memory Capacity (WMC) interval
    ...
    # 1. Plot scatter points and fitting lines for each language
    for lang in all_lang_data:
        name = lang['name']
        lang_color = lang['color']
        
        # Control Legend to show only once per language
        label_added = False 
        ...
        for m_name, fit_type in [('Hierarchical', 'log'), ('Branching', 'linear')]:
            ...
            # --- Plot raw points ---
            # s=1, alpha=0.05 to handle the massive volume of 24 languages
            ...
            try:
                if fit_type == 'log':
                    ...
                    popt, _ = curve_fit(lambda t,a,b: a*t+b, x, y)
                    yf = popt[0]*xf + popt[1]
                    ls = ':' # Dotted line for Branching to distinguish
    ...
    # 2. Plot global average thick lines (using high-contrast academic colors)
    mean_cfg = {
    ...
    # Legend configuration: placed on the right
...
if __name__ == "__main__":
    ...
    print(f"{'='*20} Cross-linguistic Cognitive Load Analysis Started {'='*20}")
    ...
    for lang_info in LANGUAGES:
        ...
        # --- Random sampling 3000 sentences ---
        if len(raw_sents_all) > SAMPLE_SIZE:
            raw_sents = random.sample(raw_sents_all, SAMPLE_SIZE)
            print(f"    [Sampling] Randomly selected {SAMPLE_SIZE} sentences from {len(raw_sents_all)}")
        else:
            raw_sents = raw_sents_all
            print(f"    [Sampling] Insufficient corpus for {SAMPLE_SIZE}, processing full set ({len(raw_sents)} sentences)")

        temp_data = []
        print(f"    [Stage 1/3] Tokenization and initial filtering...")
        for s in raw_sents:
            s = s.replace('\u3000', ' ').strip()
            if not s: continue
        ...
        print(f"    [Stats] Sentence Length Distribution:")
        print(f"        Sample size after filtering: {len(lens)}")
        print(f"        Mean: {mean_len:.2f}, Std: {std_len:.2f}")
        print(f"        Q1 (25%): {q1:.2f}, Q3 (75%): {q3:.2f}, IQR: {iqr:.2f}")
        print(f"        Lower Bound (Outlier filtering): {lower:.2f}")
        print(f"        Upper Bound (Outlier filtering): {upper:.2f}")
        
        filtered_entries = [d for d in temp_data if lower <= d['len'] <= upper]
        total_to_process = len(filtered_entries)
        print(f"    [Stage 2/3] Total sentences to process after filtering: {total_to_process}")
        ...
        print(f"    [Stage 3/3] Dependency Parsing and Complexity Calculation:")
    ...
    if combined_results_for_plot:
        print(">>> Generating cross-linguistic comparison plot...")
        plot_combined_24_lines_with_points(combined_results_for_plot)

    # -------------------------- 5. Age-Group Trend Analysis (inc. F, R^2, p stats) --------------------------
    if combined_results_for_plot:
        print(">>> Generating age-group trend analysis and performing statistical tests...")
        ...
        try:
            ...
            print("\n" + "="*40)
            print("[Regression Analysis Statistics]")
            print(f"Equation: θ = {slope:.4f} * ln(Age) + {intercept:.4f}")
        ...
        plt.close()
        print(">>> Trend analysis plot saved to figures folder.")

    print("\n" + "="*100)
    print(f"{'Corpus Name':<18} | {'Valid Sents':<12} | {'Mean Length':<15} | {'Std Length':<15}")
    print("-" * 100)
    
    all_lengths_list = []
    
    for item in combined_results_for_plot:
        # Extract sentence lengths for current corpus
    ...
    if all_lengths_list:
        ...
        print("-" * 100)
        print(f"{'TOTAL (All Corpora)':<18} | {total_cnt:<12} | {total_avg:<15.2f} | {total_std:<15.2f}")
