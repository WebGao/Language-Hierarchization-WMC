...
def plot_single_language(name, code, color, models_data):
    # Set Nature-style proportions with a compact canvas
    ...
    for m_name, m_cfg in configs.items():
        ...
        # Scatter plot: reduced size and increased transparency to handle overlapping data
    ...
    # Minimalist whitespace handling
...
def plot_combined_24_lines_with_points(all_lang_data):
    ...
    # 1. Plot points and fitting lines for each language
    for lang in all_lang_data:
        name = lang['name']
        lang_color = lang['color']
        
        # Ensure legend only displays once per language
        label_added = False 
        ...
        for m_name, fit_type in [('Hierarchical', 'log'), ('Branching', 'linear')]:
            ...
            # --- Plot raw points ---
            # s=20, alpha=0.05 to handle the large volume of data across 24 languages
            ...
            try:
                if fit_type == 'log':
                    ...
                    popt, _ = curve_fit(lambda t,a,b: a*t+b, x, y)
                    yf = popt[0]*xf + popt[1]
                    ls = ':' # Set Branching style to dotted for distinction
    ...
    # 2. Plot global mean lines (using high-contrast academic color palette)
    mean_cfg = {
...
if __name__ == "__main__":
    ...
    print(f"{'='*20} Initializing Cross-linguistic Cognitive Load Analysis {'='*20}")
    ...
    for lang_info in LANGUAGES:
        ...
        print(f"    [Stage 1/3] Pre-tokenization and initial screening...")
        for s in raw_sents:
            s = s.replace('\u3000', ' ').strip()
            if not s: continue
        ...
        for idx, entry in enumerate(filtered_entries):
            # Update progress every 5 sentences or on the last sentence
    ...
    print("\n" + "="*100)
    print(f"{'Corpus/Lang':<15} | {'Valid Sents':<12} | {'Mean Length':<15} | {'Std Length':<15}")
    print("-" * 100)
    
    for lang_res in combined_results_for_plot:
        # Extract sentence length list x from Hierarchical model data
    ...
    print("\nTask completed. Please check the 'figures' directory.")